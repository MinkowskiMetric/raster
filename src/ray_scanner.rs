...
impl Ray {
    pub fn new(origin: cgmath::Vector3<f32>, direction: cgmath::Vector3<f32>) -> Self {
        ...
        let sign = [if inv_direction.x < 0.0 { 1 } else { 0 }, if inv_direction.y < 0.0 { 1 } else { 0 }, if inv_direction.z < 0.0 { 1 } else { 0 }];
...
pub fn scan<P: Pixel + From<RgbaPixel>>(image: &mut impl SurfaceMut<P>, scene: &Scene) {
    for (x, y, pixel) in image.enumerate_pixels_mut() {
        ...
        *pixel = trace(Ray::new(scene.camera().position(), direction), 0, scene).unwrap_or(RgbaPixel::WHITE).into();
    }
}

pub fn trace(ray: Ray, depth: usize, scene: &Scene) -> Option<RgbaPixel> {
    scene.intersect_shapes(&ray)
    .into_iter()
